/**
 * @file test_sort_uint32.cpp
 * @author Cynog (github.com/Cynog)
 * @brief Main program to measure the time it takes to sort a vector of 256 uint32_t values on internal and external RAM with different types of chunks and caching.  
 * 
 */
...
int main(void) {
    ...
    while (1) {
        ...
        // sort
        timer_reset();
        sort_bubble<uint32_t>(list, n);
        t = timer_getms();

        // print sorted list
        serprint("sorted list:\n\r");
        for (uint16_t i = 0; i < n; i++) {
            serprintuint32(list[i]);
            serprint("   ");
        ...
        uint16_t addr = 0;
        for (uint16_t i = 0; i < n; i++) {
            uint32_t tmp = pgm_read_word(LIST + i);
            extram_write<uint32_t>(tmp, addr, i);
        }

        // sort
        timer_reset();
        sort_bubble_extram<uint32_t>(addr, n);
        t = timer_getms();

        // print sorted list
        serprint("sorted list:\n\r");
        for (uint16_t i = 0; i < n; i++) {
            serprintuint32(extram_read<uint32_t>(addr, i));
            serprint("   ");
        ...
        uint8_t n_chunks = 16;
        timer_reset();
        sort_bubble_extram_chunks<uint32_t>(addr, n, n_chunks, addr_sorted, false);
        t = timer_getms();

        // print sorted list
        serprint("sorted list:\n\r");
        for (uint16_t i = 0; i < n; i++) {
            serprintuint32(extram_read<uint32_t>(addr_sorted, i));
            serprint("   ");
        ...
        // sort
        timer_reset();
        sort_bubble_extram_chunks<uint32_t>(addr, n, n_chunks, addr_sorted);
        t = timer_getms();
